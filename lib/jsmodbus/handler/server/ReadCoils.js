    ...
    .init(function () {
        ...
        let _onRequest = (pdu, cb) => {
            if (pdu.length !== 5) {
                ...
                this.emit('preReadCoilsRequest', address, value);

                let mem = this.getCoils();

                if (!quantity || start + quantity > mem.length * 8) {
                    this.log.warn(`FC${fc} request outside coils boundaries: from ${start}, len ${quantity}. Expected max address ${mem.length * 8}`);
                    cb(Put().word8(0x81).word8(0x02).buffer());
                } else {
                    let val = 0;
                    let thisByteBitCount = 0;
                    let response = Put().word8(0x01).word8(Math.floor(quantity / 8) + (quantity % 8 === 0 ? 0 : 1));
