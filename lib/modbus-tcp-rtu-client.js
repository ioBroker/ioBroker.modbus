...
module.exports = stampit()
  .init(function () {
    ...
    var onSocketData = function (data) {
      ...
      while (buffer.length > 4) {
        ...
        if (crc.crc16modbus(pdu) === 0) { /* PDU is valid if CRC across whole PDU equals 0, else ignore and do nothing */
          ...
          this.logError('Wrong CRC for frame: ' + pdu.length + ' bytes')
          // reset buffer and try again
          buffer = []
          break
        }
    ...
    var onSend = function (pdu) {
      ...
      var head = Buffer.allocUnsafe(1)
      head.writeUInt8(this.unitId, 0)

      var pkt = Buffer.concat([head, pdu])

      crc16 = crc.crc16modbus(pkt)
      var tail = Buffer.allocUnsafe(2)
      tail.writeUInt16BE(crc16)

      pkt = Buffer.concat([pkt, tail])
      socket.write(pkt)
    }.bind(this)
