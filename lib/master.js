...
function Master(options, adapter) {
    ...
    function pollBinaries(device, regType, func, callback) {
        let regs = device[regType];
        if (regs.length) {
            modbusClient[func](regs.addressLow, regs.length).then(registers => {
                for (let n = 0; regs.config.length > n; n++) {
                    let id = regs.config[n].id;
                    //!!!!!! really coils for coils and discreteInputs. jsmodbus is very strange
    ...
    function pollFloatBlock(device, regType, func, block, callback) {
        ...
        modbusClient.func(regBlock.start, regBlock.count).then(buffer => {
            if (buffer.payload && buffer.payload.length) {
                ...
                adapter.log.warn(`Null buffer length ${func} for register ${regBlock.start}`);
            }
            // special case
    ...
    function writeFloatsReg(device, regType, obj, callback) {
        ...
        if (obj.native.len > 1) {
            ...
            modbusClient.writeMultipleRegisters(obj.native.address, buffer)
    ...
    function poll() {
        ...
        pollBinaries(device, 'disInputs', 'readDiscreteInputs', err => {
            if (err) {
                pollResult(startTime, err);
            } else {
                pollBinaries(device, 'coils', 'readCoils', err => {
                    if (err) {
                        pollResult(startTime, err);
                    } else {
