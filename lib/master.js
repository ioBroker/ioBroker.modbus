...
function Master(options, adapter) {
    ...
    let nextPoll;
    let pollTime;
    let intervalTimers = {}; // Store polling timers per interval
    let errorCount = 0;
    let ackObjects = {};
    ...
    function reconnect(isImmediately) {
        if (reconnectTimeout) {
            adapter.clearTimeout(reconnectTimeout);
            reconnectTimeout = null;
        }
        if (nextPoll) {
            adapter.clearTimeout(nextPoll);
            nextPoll = null;
        }
        // Clear all interval timers
        for (let interval in intervalTimers) {
            if (intervalTimers[interval]) {
                adapter.clearTimeout(intervalTimers[interval]);
                intervalTimers[interval] = null;
            }
        }
        intervalTimers = {};
        if (keepAliveTimeout) {
            adapter.clearTimeout(keepAliveTimeout);
    ...
    function pollBinariesBlock(device, regType, func, block, callback) {
        ...
        if (block >= regs.blocks.length) {
            if (options.config.readInterval) {
                return adapter.setTimeout(() => callback(), options.config.readInterval);
            }
            return callback();
        }
                ...
                .then(response => {
                    if (response.data && response.data.length) {
                        for (let n = regBlock.startIndex; n < regBlock.endIndex; n++) {
                            ...
                            if (
                                options.config.alwaysUpdate ||
                                ackObjects[id] === undefined ||
                    ...
                    if (options.config.readInterval) {
                        adapter.setTimeout(
                            ...
                            options.config.readInterval,
                        );
                    } else {
                        adapter.setTimeout(() => pollBinariesBlock(device, regType, func, block + 1, callback), 0);
                    }
                })
    ...
    function pollBinariesBlockWithKeepAlive() {
        if (options.config.keepAliveInterval > 0) {
            if (keepAliveTimeout) {
                keepAliveTimeout && adapter.clearTimeout(keepAliveTimeout);
                keepAliveTimeout = null;
            }

            if (modbusClient) {
                modbusClient['readDiscreteInputs'](deviceIds[0], 0, 1)
                    .then(response => adapter.log.silly(`Keep alive response = ${JSON.stringify(response)}`))
                    .catch(() => {});

                keepAliveTimeout = adapter.setTimeout(() => {
                    keepAliveTimeout = null;
                    pollBinariesBlockWithKeepAlive();
                }, options.config.keepAliveInterval);
            } else {
                adapter.log.debug(`Poll canceled, because no connection`);
            }
        } else {
    ...
    function pollBinariesBlocks(device, regType, func, callback) {
        let regs = device[regType];
        if (regs.length) {
            pollBinariesBlock(device, regType, func, 0, err => callback(err));
        } else {
            callback();
        }
    }
    ...
    function pollFloatBlock(device, regType, func, block, callback) {
        ...
        if (modbusClient) {
            modbusClient[func](regs.deviceId, regBlock.start, regBlock.count)
                .then(response => {
                    showDebug &&
                        adapter.log.debug(`[DevID_${regs.deviceId}/${regType}] Poll address ${regBlock.start} DONE`);
                            ...
                            if (regs.config[n].isScale) {
                                ...
                                try {
                                    ...
                                    if (regs.config[n].formula) {
                                        ...
                                        showDebug &&
                                            adapter.log.debug(`[${prefixAddr}] _Formula = ${regs.config[n].formula}`);
                                        try {
                                            const scaleAddress = parseInt(
                                                regs.config[n].formula.substring(
                                                    regs.config[n].formula.indexOf('sf[') + 4,
                                                ),
                                            );
                                            ...
                                            adapter.log.warn(
                                                `[${prefixAddr}] Calculation: eval(${regs.config[n].formula}) not possible: ${e}`,
                                            );
                                        }
                                    ...
                                    adapter.log.error(`Can not set value for [DevID_${regs.deviceId}]: ${err.message}`);
                                }
                            }
                        }
                    ...
                    if (options.config.maxBlock < 2 && regs.config[regBlock.startIndex].cw) {
                        ...
                        if (options.config.readInterval) {
                            ...
                            adapter.setTimeout(() => pollFloatBlock(device, regType, func, block + 1, callback), 0);
                        }
                    }
                })
                .catch(err => callback(err));
    ...
    function writeWithPause(tasks, cb) {
        if (!tasks || !tasks.length) {
            ...
            modbusClient
                .writeSingleRegister(task.deviceId, task.address, task.buffer)
                .then(() => {
                    if (options.config.writeInterval) {
                        adapter.setTimeout(writeWithPause, options.config.writeInterval, tasks, cb);
                    } else {
    ...
    function writeFloatsReg(device, regType, obj, callback) {
        ...
        if (obj.native.len) {
            ...
            if (options.config.onlyUseWriteMultipleRegisters || !options.config.doNotUseWriteMultipleRegisters) {
                ...
                modbusClient
                    .writeMultipleRegisters(regs.deviceId, obj.native.address, buffer)
                    .then(() => callback())
                    .catch(err => {
    ...
    function pollResult(startTime, deviceId, err, cb) {
        if (err) {
            ...
            adapter.log.warn(`[DevID_${deviceId}] Poll error count: ${errorCount} code: ${JSON.stringify(err)}`);
            adapter.setState('info.connection', false, true);

            if (errorCount > 12 * deviceIds.length) {
    ...
    function pollDeviceRegisters(device, registerTypes, callback) {
        adapter.log.debug(
            `[DevID_${device.coils.deviceId}] Poll registers ${registerTypes.join(', ')} start ---------------------`,
        );
        ...
        function pollNextRegisterType() {
            if (currentTypeIndex >= registerTypes.length) {
                // All register types polled successfully
                if (requestTimer) {
                    adapter.clearTimeout(requestTimer);
                    requestTimer = null;
                    if (connected && !isStop) {
                        pollResult(startTime, device.coils.deviceId, null, callback);
                    }
                }
                return;
            }

            const regType = registerTypes[currentTypeIndex];
            currentTypeIndex++;
            ...
            if (regType === 'coils') {
                pollBinariesBlocks(device, 'coils', 'readCoils', err => {
                    if (err) {
                        ...
                        pollNextRegisterType();
                    }
                });
            } else if (regType === 'holdingRegs') {
    ...
    function getRegisterGroups() {
        ...
        for (let deviceId of deviceIds) {
            ...
            // Check coils
            if (device.coils && device.coils.config) {
                for (let config of device.coils.config) {
                    const interval = parseInt(config.pollInterval, 10) || options.config.poll;
                    if (!groups[interval]) {
                        groups[interval] = {};
                    }
                    if (!groups[interval][deviceId]) {
                        groups[interval][deviceId] = { coils: false, holdingRegs: false };
                    }
                    groups[interval][deviceId].coils = true;
                }
            }

            // Check holding registers
            if (device.holdingRegs && device.holdingRegs.config) {
                for (let config of device.holdingRegs.config) {
                    const interval = parseInt(config.pollInterval, 10) || options.config.poll;
                    if (!groups[interval]) {
                        groups[interval] = {};
                    }
                    ...
                    groups[interval][deviceId].holdingRegs = true;
                }
            }
        }
    ...
    function pollRegisterGroup(interval, deviceGroup) {
        const deviceIdsToPoll = Object.keys(deviceGroup);
        let deviceIndex = 0;

        function pollNextDevice() {
            if (deviceIndex >= deviceIdsToPoll.length) {
                // All devices processed, schedule next poll for this interval
                if (deviceIdsToPoll.some(id => !devices[id].err)) {
                    intervalTimers[interval] = adapter.setTimeout(() => {
                        intervalTimers[interval] = null;
                        pollRegisterGroup(interval, deviceGroup);
                    }, interval);
                }
                return;
            }

            const deviceId = deviceIdsToPoll[deviceIndex];
            const device = devices[deviceId];
            const registerTypes = [];

            // Determine which register types to poll for this device in this interval group
            if (deviceGroup[deviceId].coils && device.coils && device.coils.config.length > 0) {
                // Only poll coils that have this specific interval
                const coilsForInterval = device.coils.config.filter(
                    c => (parseInt(c.pollInterval, 10) || options.config.poll) === interval,
                );
                if (coilsForInterval.length > 0) {
                    registerTypes.push('coils');
                }
            }

            if (deviceGroup[deviceId].holdingRegs && device.holdingRegs && device.holdingRegs.config.length > 0) {
                // Only poll holding registers that have this specific interval
                const holdingRegsForInterval = device.holdingRegs.config.filter(
                    c => (parseInt(c.pollInterval, 10) || options.config.poll) === interval,
                );
                if (holdingRegsForInterval.length > 0) {
                    registerTypes.push('holdingRegs');
                }
            }

            if (registerTypes.length === 0) {
                deviceIndex++;
                setImmediate(pollNextDevice);
                return;
            }

            pollDeviceRegisters(device, registerTypes, err => {
                devices[deviceId].err = err;
                deviceIndex++;

                // Wait between devices if configured
                if (options.config.waitTime) {
                    adapter.setTimeout(pollNextDevice, options.config.waitTime);
                } else {
                    setImmediate(pollNextDevice);
                }
            });
    ...
    function poll(i, cb) {
        ...
        if (i >= deviceIds.length) {
            ...
            pollDevice(devices[deviceIds[i]], err => {
                ...
                if (options.config.waitTime) {
                    adapter.setTimeout(poll, options.config.waitTime, i + 1, cb);
                } else {
                    adapter.setTimeout(poll, 0, i + 1, cb);
                }
            });
        }
    }

    function startIntervalBasedPolling() {
        const registerGroups = getRegisterGroups();

        // Start polling for each interval group
        for (let interval in registerGroups) {
            const deviceGroup = registerGroups[interval];
            adapter.log.debug(
                `Starting polling for interval ${interval}ms with devices: ${Object.keys(deviceGroup).join(', ')}`,
            );
            pollRegisterGroup(parseInt(interval, 10), deviceGroup);
        }
    }
    ...
    function send() {
        ...
        try {
            if (type === 'coils') {
                ...
                promise = modbusClient
                    .writeSingleCoil(objects[id].native.deviceId, objects[id].native.address, !!val)
                    .then(
                        _response =>
                            showDebug &&
                ...
                // FC16
                if (
                    options.config.onlyUseWriteMultipleRegisters ||
                    ...
                    promise = modbusClient
                        .writeMultipleRegisters(objects[id].native.deviceId, objects[id].native.address, hrBuffer)
                        .then(
                            _response =>
                                showDebug &&
                                adapter.log.debug(`Write successfully [${objects[id].native.address}]: ${val}`),
                        ...
                        .catch(err => {
                            adapter.log.error(
                                `Cannot write multiple registers [${objects[id].native.address}]: ${JSON.stringify(err)}`,
                            );
                            // still keep on communication
                            !isStop && !reconnectTimeout && reconnect(true);
                        });
                    ...
                    promise = modbusClient
                        .writeSingleRegister(objects[id].native.deviceId, objects[id].native.address, buffer)
                        .then(
                            _response =>
                                showDebug &&
                                adapter.log.debug(`Write successfully [${objects[id].native.address}]: ${val}`),
                        ...
                        .catch(err => {
                            adapter.log.error(
                                `Cannot write single register [${objects[id].native.address}]: ${JSON.stringify(err)}`,
                            );
                            // still keep on communication
                            !isStop && !reconnectTimeout && reconnect(true);
                        });
        ...
        if (Object.keys(sendBuffer).length) {
            promise.then(() => {
                if (options.config.writeInterval) {
                    adapter.setTimeout(send, options.config.writeInterval);
                } else {
                    adapter.setTimeout(send, 0);
                }
            });
        }
    }
        ...
        if (objects[id].native.regType === 'coils' || objects[id].native.regType === 'holdingRegs') {
            if (!objects[id].native.wp) {
                ...
                adapter.setTimeout(() => {
                    ...
                    adapter.setState(
                        id,
                        ackObjects[_id] ? ackObjects[_id].val : null,
                        true,
                        err =>
    ...
    this.close = () => {
        ...
        if (connectTimer) {
            adapter.clearTimeout(connectTimer);
            connectTimer = null;
        }
    ...
    (function _constructor() {
        ...
        adapter.getStatesAsync('info.clients.*').then(async states => {
        ...
        if (options.config.type === 'tcp') {
            ...
            try {
                ...
                adapter.log.error(
                    `Cannot open port "${serial.comName}" [${parseInt(serial.baudRate, 10) || 9600}]: ${e}`,
                );
            }
        } else {
        ...
        modbusClient
            .on('connect', () => {
                ...
                // Check if any registers have custom poll intervals
                let hasCustomIntervals = false;
                for (let deviceId of deviceIds) {
                    const device = devices[deviceId];
                    if (device.coils && device.coils.config) {
                        for (let config of device.coils.config) {
                            if (config.pollInterval && parseInt(config.pollInterval, 10) !== options.config.poll) {
                                hasCustomIntervals = true;
                                break;
                            }
                        }
                    }
                    if (!hasCustomIntervals && device.holdingRegs && device.holdingRegs.config) {
                        for (let config of device.holdingRegs.config) {
                            if (config.pollInterval && parseInt(config.pollInterval, 10) !== options.config.poll) {
                                hasCustomIntervals = true;
                                break;
                ...
                if (hasCustomIntervals) {
                    adapter.log.info('Using interval-based polling due to custom poll intervals');
                    startIntervalBasedPolling();
                } else {
                    adapter.log.debug('Using legacy polling (no custom intervals)');
                    poll();
                }
            ...
            .on('disconnect', () => {
                ...
                reconnectTimeout = reconnectTimeout || adapter.setTimeout(reconnect, 1000);
            });
        ...
        modbusClient.on('trashCurrentRequest', err => {
            ...
            if (!reconnectTimeout) {
                reconnectTimeout = adapter.setTimeout(reconnect, 1000);
            }
        });
    })();
