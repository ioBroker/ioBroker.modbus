    ...
    function send() {
        ...
        if (type === 'coils') {
            ...
            if (objects[id].native.len > 1) {
                ...
                modbusClient.writeMultipleRegisters(objects[id].native.deviceId, objects[id].native.address, hrBuffer, (err, response) =>
                    adapter.log.debug('Write successfully [' + objects[id].native.address + ']: ' + val)
                ).catch(err => {
                    adapter.log.error('Cannot write [' + objects[id].native.address + ']: ' + JSON.stringify(err));
                    // still keep on communication
                ...
                modbusClient.writeSingleRegister(objects[id].native.deviceId, objects[id].native.address, buffer).then(response =>
                    adapter.log.debug('Write successfully [' + objects[id].native.address + ': ' + val)
                ).catch(err => {
                    adapter.log.error('Cannot write [' + objects[id].native.address + ']: ' + JSON.stringify(err));
                    // still keep on communication
