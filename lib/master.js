...
function Master(options, adapter) {
    ...
    function reconnect(isImmediately) {
        ...
        connectTimer = connectTimer || adapter.setTimeout(() => {
            connectTimer = null;
    ...
    function pollBinariesBlock(device, regType, func, block, callback) {
        ...
        if (showDebug) {
            adapter.log.debug(`[DevID_${regs.deviceId}/${regType}] Poll address ${regBlock.start} - ${regBlock.count} bits`);
        }
        ...
        if (modbusClient) {
            modbusClient[func](regs.deviceId, regBlock.start, regBlock.count)
                .then(response => {
                    ...
                    if (options.config.readInterval) {
                        adapter.setTimeout(() => pollBinariesBlock(device, regType, func, block + 1, callback), options.config.readInterval);
                    } else {
                ...
                .then(response => {
                    ...
                    if (options.config.maxBlock < 2 && regs.config[regBlock.startIndex].cw) {
                        ...
                        writeFloatsReg(device, regType, objects[regs.config[regBlock.startIndex].fullId], () => {
                            if (options.config.readInterval) {
                                adapter.setTimeout(() => pollFloatBlock(device, regType, func, block + 1, callback), options.config.readInterval);
                            } else {
    ...
    function writeFloatsRegs(device, regType, i, callback) {
        ...
        writeFloatsReg(device, regType, objects[id], () => {
            if (options.config.readInterval) {
                adapter.setTimeout(() => writeFloatsRegs(device, regType, i + 1, callback), options.config.readInterval);
            } else {
    ...
    function pollResult(startTime, deviceId, err, cb) {
        if (err) {
            ...
            if (errorCount > 12 * deviceIds.length) { // 2 re-connects did not help, restart adapter
    ...
    (function _constructor() {
        ...
        adapter.getStatesAsync('info.clients.*')
            .then(async states => {
