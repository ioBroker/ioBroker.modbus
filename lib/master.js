                        ...
                        for (let n = regBlock.startIndex; n < regBlock.endIndex; n++) {
                            if (regs.config[n].type !== 'string' && regs.config[n].type !== 'stringle' && regs.config[n].isScale) {
                                ...
                                if (regs.config[n].formula) {
                                    ...
                                    try {
                                        let scaleAdress = parseInt(regs.config[n].formula.substring(regs.config[n].formula.indexOf("sf[") + 4));
                                        if (scaleAdress !== null && !isNaN(scaleAdress)) {
                                            adapter.log.warn('Calculation of a scaleFactor which is based on another scaleFactor seems strange. Please check the config for adress ' + regs.config[n].address + ' !');
                                        }
                            ...
                            if (regs.config[n].type !== 'string' && regs.config[n].type !== 'stringle') {
                                ...
                                if (regs.config[n].formula) {
                                    ...
                                    try {
                                            ...
                                            m.forEach(sf => {
                                                ...
                                                if (num) {
                                                    ...
                                                    if (scaleAddress !== null && !isNaN(scaleAddress)) {
                                                        ...
                                                        adapter.log.debug('Scale factor address is = ' + scaleAddress);
                                                        adapter.log.debug('Scale factor address is inside current read range = ' + (scaleAddress > regBlock.start && scaleAddress < (regBlock.start + regBlock.count)));
                                                        ...
                                                        if (scaleAddress < regBlock.start || scaleAddress > (regBlock.start + regBlock.count)) {
                                                            ...
                                                            adapter.log.warn(`The current range for reading the values was from adress ${regBlock.start} up to adress ${regBlock.start + regBlock.count}!`);
                                                            adapter.log.warn('Please make sure to configure the read process that both adresses are read in the same block!');
                                                            adapter.log.warn(`The used scaleFactor from address ${scaleAddress} is not inside the same read block as the parameter on address ${regs.config[n].address}`);
                                                        }
                                                    }
                                                }
                                            });
                                        ...
                                        const func = new Function('x', 'sf', 'return ' + regs.config[n].formula);
                                        val = func(val, scaleFactors[regs.deviceId]);
                                        adapter.log.debug(`Calculation result = ${val}, type = ${typeof val}`);
    ...
    function writeFloatsReg(device, regType, obj, callback) {
        ...
        if (obj.native.len) {
            ...
            if (options.config.onlyUseWriteMultipleRegisters || !options.config.doNotUseWriteMultipleRegisters) {
                ...
                let buffer = common.writeValue(obj.native.type, val, 1);
                ...
                modbusClient.writeSingleRegister(regs.deviceId, obj.native.address, buffer)
                    .then(response => callback())
                    .catch(err => {
                        adapter.log.error('Cannot write single registers_: ' + JSON.stringify(err));
                        callback(err);
                    });
            }
        } else {
