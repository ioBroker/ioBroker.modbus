                        ...
                        for (let n = regBlock.startIndex; n < regBlock.endIndex; n++) {
                            ...
                            if (regs.config[n].type !== 'string' && regs.config[n].type !== 'stringle') {
                                ...
                                if (regs.config[n].formula) {
                                    ...
                                    adapter.log.debug('Formula = ' + regs.config[n].formula);
                                    try {
                                        // scaleAdress is used only for debug output
                                        let scaleAdress = parseInt(regs.config[n].formula.substring(regs.config[n].formula.indexOf('sf[') + 4));

                                        if (scaleAdress !== null && !isNaN(scaleAdress)) {
                                            // it seems that the current formula uses a scaleFactor, therefore check the validity
                                            adapter.log.debug('Scalefactor adress is = ' + scaleAdress);
                                            adapter.log.debug('Scalefactor adress is inside current read range = ' + (scaleAdress > regBlock.start && scaleAdress < (regBlock.start + regBlock.count)));
                                            // check if the scaleFactor adress is in the current read block or outside this block
                                            if (scaleAdress < regBlock.start || scaleAdress > (regBlock.start + regBlock.count)) {
                                                // the scaleFactor adress is not in the current read block. So it cannot ensured that the values are in sync / valid
                                                ...
                                                adapter.log.warn('The used scaleFactor from address ' + scaleAdress + ' is not inside the same read block as the parameter on address ' + regs.config[n].address);
                                            }
                                        }
                                        // calculate value from formula or report an error
                                        const func = new Function('x', 'sf', 'return ' + regs.config[n].formula);
