...
function Master(options, adapter) {
    ...
    function pollFloatBlock(device, regType, func, block, callback) {
        ...
        adapter.log.debug(`Poll ${regType} DevID(${regs.deviceId}) address ${regBlock.start} - ${regBlock.count} registers`);
        if (modbusClient) {
            modbusClient[func](regs.deviceId, regBlock.start, regBlock.count)
                .then(response => {
                    if (response.payload && response.payload.length) {
                        ...
                        for (let n = regBlock.startIndex; n < regBlock.endIndex; n++) {
                            if (regs.config[n].type !== 'string' && regs.config[n].type !== 'stringle' && regs.config[n].isScale) {
                                ...
                                if (regs.config[n].formula) {
                                    adapter.log.debug(`[${regType}/${regs.config[n]._address}] _Input Value = ${val}`);
                                    adapter.log.debug(`[${regType}/${regs.config[n]._address}] _Formula = ${regs.config[n].formula}`);
                                    try {
                                        ...
                                        if (scaleAddress !== null && !isNaN(scaleAddress)) {
                                            adapter.log.warn(`[${regType}/${regs.config[n]._address}] Calculation of a scaleFactor which is based on another scaleFactor seems strange. Please check the config for adress ${regs.config[n].address} !`);
                                        }
                                        ...
                                        adapter.log.warn(`[${regType}/${regs.config[n]._address}] Calculation: eval(${regs.config[n].formula}) not possible: ${e}`);
                                    }
                                ...
                                adapter.log.debug(`[${regType}/${regs.config[n]._address}] Scale factor value stored from this address = ${val}`);
                            }
                        }
                        ...
                        for (let n = regBlock.startIndex; n < regBlock.endIndex; n++) {
                            ...
                            if (regs.config[n].type !== 'string' && regs.config[n].type !== 'stringle') {
                                // If value must be calculated with formula
                                if (regs.config[n].formula) {
                                    adapter.log.debug(`[${regType}/${regs.config[n]._address}] Input Value = ${val}`);
                                    adapter.log.debug(`[${regType}/${regs.config[n]._address}] Formula = ${regs.config[n].formula}`);
                                    try {
                                            ...
                                            m.forEach(sf => {
                                                ...
                                                if (num) {
                                                    ...
                                                    if (scaleAddress !== null && !isNaN(scaleAddress)) {
                                                        ...
                                                        adapter.log.debug(`[${regType}/${regs.config[n]._address}] Scale factor address is = ${scaleAddress}`);
                                                        adapter.log.debug(`[${regType}/${regs.config[n]._address}] Scale factor address is inside current read range = ${scaleAddress > regBlock.start && scaleAddress < (regBlock.start + regBlock.count)}`);
                                                        ...
                                                        if (scaleAddress < regBlock.start || scaleAddress > (regBlock.start + regBlock.count)) {
                                                            // the scaleFactor address is not in the current read block. So it cannot ensured that the values are in sync / valid
                                                            adapter.log.warn(`[${regType}/${regs.config[n]._address}] The current range for reading the values was from address ${regBlock.start} up to address ${regBlock.start + regBlock.count}!`);
                                                            adapter.log.warn(`[${regType}/${regs.config[n]._address}] Please make sure to configure the read process that both adresses are read in the same block!`);
                                                            adapter.log.warn(`[${regType}/${regs.config[n]._address}] The used scaleFactor from address ${scaleAddress} is not inside the same read block as the parameter on address ${regs.config[n].address}`);
                                                        }
                                                    }
                                                }
                                            });
                                        ...
                                        const func = new Function('x', 'sf', 'return ' + regs.config[n].formula);
                                        val = func(val, scaleFactors[regs.deviceId]);
                                        adapter.log.debug(`[${regType}/${regs.config[n]._address}] Calculation result = ${val}, type = ${typeof val}`);
    ...
    function writeWithPause(tasks, cb) {
        if (!tasks || !tasks.length) {
            ...
            modbusClient.writeSingleRegister(task.deviceId, task.address, task.buffer)
                .then(() => setTimeout(writeWithPause, options.config.writeInterval, tasks, cb))
                .catch(err => {
