...
function Master(options, adapter) {
    ...
    function pollDevice(device, callback) {
        ...
        const totalPolls = 4; // disInputs, coils, inputRegs, holdingRegs

        function checkPollComplete() {
            completedPolls++;
            if (completedPolls >= totalPolls) {
                if (requestTimer) {
                    ...
                    if (connected && !isStop) {
                        ...
                        if (pollErrors.length > 0 && pollErrors.length < totalPolls) {
                            adapter.log.warn(
                                `[DevID_${device.coils.deviceId}] Some register types failed but continuing: ${pollErrors.length}/${totalPolls} errors`,
                            );
                        }
                        pollResult(startTime, device.coils.deviceId, error, callback);
                    }
                }
            }
        }
    ...
    (function _constructor() {
        ...
        if (options.config.type === 'tcp') {
            ...
            try {
                ...
                if (options.config.ssl?.cert) {
                    sslConfig.cert = options.config.ssl.cert;
                }

                if (options.config.ssl?.key) {
                    sslConfig.key = options.config.ssl.key;
                }

                if (options.config.ssl?.ca) {
                    sslConfig.ca = options.config.ssl.ca;
                }
                ...
                adapter.log.error(`Cannot connect to SSL "${tcp.bind}:${parseInt(tcp.port, 10) || 502}": ${e}`);
            }
